...
> & {
  items: ToolDockItem[];
  /** Current active/selected index */
  activeIndex?: number;
  /** Selection callback when clicking an item */
...
const DockTile = React.memo(function DockTile({
  ...
  const near = useTransform(pointer, (x) => {
    if (still) return 0;
    return swellAt(layout.current, index, x);
  });
  ...
  const scale = useTransform(swell, (v) => (isActive ? 1.08 : 1) + v * magnification);
  const y = useTransform(swell, (v) => -v * LIFT * (layout.current.width || 56) - (isActive ? 4 : 0));
  const rotate = useTransform(swell, (v) => (isActive ? 0 : lean * (1 - v)));
  ...
  return (
    <motion.li
      ...
      className="relative flex cursor-pointer pointer-events-auto"
      style={{
        // At rest each tile covers the one after it.
        zIndex: isActive ? count + 10 : count - index,
        marginLeft:
...
export function ToolDock({
  ...
  const [text, setText] = React.useState(
    (activeIndex !== undefined ? items[activeIndex]?.label : items[0]?.label) ?? ""
  );
  ...
  React.useEffect(() => {
    ...
    const measure = () => {
      ...
      if (activeIndex !== undefined && layout.current.centers[activeIndex] !== undefined) {
        setAt(layout.current.centers[activeIndex]);
        setText(items[activeIndex]?.label ?? "");
      }
    };
    measure();
  ...
  // Update tooltip position if activeIndex changes externally
  React.useEffect(() => {
    if (activeIndex !== undefined && items[activeIndex]) {
      setText(items[activeIndex].label);
      if (layout.current.centers[activeIndex] !== undefined) {
        setAt(layout.current.centers[activeIndex]);
      }
    }
  }, [activeIndex, items]);
  ...
  const tooltipVisible = active !== null || activeIndex !== undefined;
    ...
    >
      ...
      >
        <span
          ...
          className={cn(
            "pointer-events-none absolute bottom-[calc(var(--tool-dock-size)*(1+var(--tool-dock-rise))+0.625rem)] left-0 z-50 whitespace-nowrap rounded-md border border-white/20 bg-[#071118]/90 px-2.5 py-1 text-[12px] sm:text-[13px] font-mono font-semibold text-emerald-300 shadow-xl backdrop-blur-md opacity-0 ease-[cubic-bezier(0.22,1,0.36,1)] data-[state=open]:opacity-100 motion-reduce:transition-none",
            glide
        ...
        >
          ...
          <span className="absolute -bottom-[5px] left-1/2 -ml-[4.5px] size-[9px] rotate-45 rounded-br-[2px] border-r border-b border-white/20 bg-[#071118]" />
        </span>
...
export function ToolDockTile({
  ...
  return (
    <div
      ...
      className={cn(
        "relative grid size-full place-items-center overflow-hidden rounded-[23%] bg-slate-900/90 text-white shadow-[0_0_0_0.5px_rgb(255_255_255/0.1),0_2px_4px_rgb(0_0_0/0.4),0_0_1px_rgb(255_255_255/0.2)]",
        "after:pointer-events-none after:absolute after:inset-0 after:rounded-[inherit] after:bg-gradient-to-b after:from-white/20 after:to-transparent after:shadow-[inset_0_0_0_0.5px_rgb(255_255_255/0.2),inset_0_1px_0_rgb(255_255_255/0.4)]",
        className,
      )}
      {...props}
    >
