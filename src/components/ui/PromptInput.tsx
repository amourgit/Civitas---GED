...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    const handleFilesChosen = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!fileList) return;
      const files: File[] = Array.from(fileList as unknown as File[]).filter((f: File) => f.type.startsWith("image/"));
      e.target.value = ""; 
        ...
        >
          ...
          >
            ...
            <button
              ...
              className={cn(
                "absolute inset-x-0 top-0 z-[1] cursor-text pl-4 pr-12 py-[15px] text-left text-sm font-medium leading-[17px] text-muted-foreground/80 outline-none truncate",
