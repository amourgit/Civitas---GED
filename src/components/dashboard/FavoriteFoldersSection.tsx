...
export function FavoriteFoldersSection({
  ...
  // Use the actual existing favorite folders from the app
  const favoriteFolders = React.useMemo(() => {
    const favs = initialFolders.filter(f => f.isFavorite);
    return favs.length >= 2 ? favs.slice(0, 4) : initialFolders.slice(2, 6);
  }, []);
  ...
  return (
    <section className="w-full flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="px-1.5 py-0.5 rounded bg-white/10 text-white/75 text-[10px] font-mono">
            {favoriteFolders.length} dossiers
          </span>
        </div>
        <button 
          type="button"
      ...
      <DocumentGrid
        ...
        onOpenFolder={handleSelect}
        onPreviewSpecial={() => {}}
        onShare={() => {}}
        onToggleFavorite={() => {}}
        onViewProperties={handleSelect}
        onDelete={() => {}}
      />
    </section>
  );
