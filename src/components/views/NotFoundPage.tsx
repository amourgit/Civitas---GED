...
function MessageDisplay() {
  ...
  const handleGoBack = () => {
    if (window.history.length > 1) {
      // Attache un listener ponctuel sur popstate pour forcer l'actualisation complète de la page précédente
      ...
      // Sécurité si l'historique était sur la même page
      setTimeout(() => {
        window.location.reload();
      }, 150);
    } else {
  ...
  const handleGoHome = () => {
    // Redirige vers la racine et force l'actualisation complète
    window.location.href = "/";
  };
  ...
  useEffect(() => {
    ...
    const renderFigures = () => {
      ...
      stickFigures.forEach((figure, index) => {
        ...
        stick.referrerPolicy = 'no-referrer';
...
function CircleAnimation() {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const context = canvas.getContext('2d');
      if (context) {
        if ('reset' in context && typeof (context as { reset?: () => void }).reset === 'function') {
          (context as { reset: () => void }).reset();
        } else {
          context.setTransform(1, 0, 0, 1, 0, 0);
          context.clearRect(0, 0, canvas.width, canvas.height);
        }
